...
void vEstimationSystemInit(SystemState* pSystemState, TelemetryData* pTelemetryData, MotorCommands* pMotorCommands){
    ...
    pSystemState = pSystemState;
    pTelemetryData = pTelemetryData;
 ...
 * 
 */
void vEstimationSystemComputeDynamicModel(void){
    float32_t Ts = 0.1f;
    float32_t r = 0.035f;
    float32_t L = 0.1f;
    ...
    fX_hat_priori[0] = pSystemState->fStateVector[0] + pSystemState->fStateVector[1] * Ts + 0.5f * pSystemState->fStateVector[2] * Ts * Ts;
    fX_hat_priori[1] = (r / 2.0f) * cosf(pSystemState->fStateVector[6]) * (u[0] + u[1]);
    fX_hat_priori[2] = pSystemState->fStateVector[2];
    fX_hat_priori[3] = pSystemState->fStateVector[3] + pSystemState->fStateVector[4] * Ts + 0.5f * pSystemState->fStateVector[5] * Ts * Ts;
    fX_hat_priori[4] = (r / 2.0f) * sinf(pSystemState->fStateVector[6]) * (u[0] + u[1]);
    fX_hat_priori[5] = pSystemState->fStateVector[5];
    fX_hat_priori[6] = pSystemState->fStateVector[6] + pSystemState->fStateVector[7] * Ts;
    fX_hat_priori[7] = (r / L) * (u[1] - u[0]);


    // Compute transition matrix jacobian
    float32_t *x = pSystemState->fStateVector;
...
void vEstimationSystemEkfEstimate(void){
...
void vEstimationSystemComputeEstimate(void){
    static char cFirstIteration = 1;

    if(cFirstIteration){
        vEstimationSystemComputeDynamicModel();
